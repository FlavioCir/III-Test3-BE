...
public static void inserisciEvento(String titolo, LocalDate dataEvento, String descrizione, TipoEvento tipoEvento, int numeroMaxPartecipanti, Location location, Partecipazione partecipazione) {
		...
		try {
			...
			e.setPartecipazione(new HashSet<>() {{
				add(partecipazione);
			}});
			t.begin();
			em.persist(e);
